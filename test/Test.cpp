...
int test(std::ostream& outStream) {
...

  {
    ...
    auto it1 = m.find(s1);
    auto it2 = m.find(s2);
    if (!(it1 != m.end() && it1->first == s1 && it1->second == 1 &&
          it2 != m.end() && it2->first == s2 && it2->second == 2)) {
  ...
  constexpr xg::Guid guidFromStringConstexpr(
      "0102030405060708090a0b0c0d0e0fdd");
  ...
  if (!std::equal(guidFromBytes.bytes().begin(), guidFromBytes.bytes().end(),
                  bytes.begin())) {
